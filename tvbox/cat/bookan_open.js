...
async function category(tid, pg, filter, extend) {
    pg = pg || 1;
    if (pg == 0) pg = 1;
    let content = await request(`${url}/voice/book/list?instance_id=27279&page=${pg}&category_id=${tid}&num=24`);
...
async function search(wd, quick, pg) {
    pg = pg || 1;
    if (pg == 0) pg = 1;
    let content = await request(`https://es.bookan.com.cn/api/v3/voice/book?instanceId=27279&keyword=${wd}&pageNum=${pg}&limitNum=20`);
    let data = JSON.parse(content).data;
    let books = [];
    for (const book of data.list) {
        books.push({

var rule = {
    ...
    lazy: typeof MY_NAME !== "undefined" ? $js.toString(() => {
        ...
        if (list.length === '1') {
            ...
            input.url = $(a, 1).select(list => {
                    input = input.match(/\d+/)[0] - 1
                    return $(JSON.parse(pdfh(list[input], '.dplayer&&config')).video.url).lazyRule(() => input.startsWith("http") ? input : "https://hls.vdtuzv.com" + input)
                }, list)
        }
    }) : void 0,
    play_parse: true,

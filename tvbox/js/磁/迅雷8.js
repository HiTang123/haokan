var rule = {
    title:'迅雷电影天堂',
    host:'https://xunlei8.org',
    url:'/fyclass-fyfilter-fypage-30.html',
    searchUrl:'/s/**.html',
    detailUrl:'',
    searchable:1,
    quickSearch:1,
    class_name:'电影&电视剧&排行榜&最近更新',
    class_url:'list&tv&rank&news',
    ...
    limit:6,
    double:false,
    图片来源:'@Referer=https://xunlei8.org',
    推荐:$js.toString(()=>{
        let d=[];
    ...
    二级:$js.toString(()=>{                                
        ...
        pdfa(html,'ul&&div').forEach(i=>{
            let b=pdfh(i,'a&&href')
            if(b.startsWith('thunder:')){
                a.push(pdfh(i,'a&&Text')+'$'+pdfh(i,'a&&href'))
            }
        });    
        VOD.vod_play_from='迅雷电影';    

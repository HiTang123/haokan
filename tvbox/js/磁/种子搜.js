...
host:'', 
searchUrl:'https://bt-api.apiuno.com/api?engin=zhongziso&key_word=**&action=search&fuzzy=true&page_count=fypage&sort_by=default&reverse=true', 
detailUrl:'fyid',
searchable:2, 
...
二级: $js.toString(() => {
  if (orId === 'info') {
    ...
    VOD.vod_play_from= '种子搜';
    VOD.vod_play_url=input;
  }
  }),
搜索:`js:
		let list = JSON.parse(request(input)).result;
		let d = [];
		for (let it of list) {
			d.push({
				title: it.title,

var rule = {
    ...
    lazy: `js:
        ...
        if (flag[0]=='parse') {
            input = { jx: 0, parse: 1, url: HOST+'/'+flag[1]}
        } else if(flag[0]=='jx') {   
            /*let jxurl="http://cfss.cc/Cf/hy/"+flag[1]+".flv";
            let a=request(jxurl);
            let b=a.match(/(?<=url: ').*?(?=',)/)[0];   
            let c=b.startsWith('//') ? "http:"+b : b;  */  
            input = { jx: 0, parse: 0, url: 'http://127.0.0.1:35456/huya/'+flag[1] }
        }
    `,
    推荐: '*',
    一级: `js:
        ...
        jo.forEach(it => {
                d.push({
                    url: it.profileRoom+'#'+it.introduction,
                    title: it.introduction,
                    img: it.screenshot,
                    desc: (it.totalCount>=10000?(it.totalCount/10000).toFixed(1)+'万':it.totalCount)+'•'+it.nick,
                })
        });
        setResult(d);
    `,
    ...
    搜索: `js:
        ...
        jo.forEach(it => {
            d.push({
                url: it.room_id+'#'+it.gameName,
                title: it.gameName,
                img: it.game_screenshot,
                desc: (it.game_total_count>=10000?(it.game_total_count/10000).toFixed(1)+'万':it.game_total_count)+'•'+it.game_nick,
            })
        });
        setResult(d);
    `,

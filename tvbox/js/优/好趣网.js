var rule = {
    ...
    lazy: `js:
        let flag='outplay';
        var url=pdfh(request(input),"body&&script&&Html").split("$")[1];
        if(url.includes(flag)){
            input = {parse:1,jx:0,url:url.split('id=')[1]}
        }else{
            input = url
        }
        `,
    ...
    二级: {
        ...
        'lists': `js:
            ...
            function getLists(html){
                let src=pdfa(html,".tab-list-syb&&li");
                let list=[];
                src.forEach(function(it){
                    let title=pdfh(it,".s&&Text");
                    let url="http://m.haoqu99.com/e/extend/tv.php?id="+pdfh(it,".tab-item&&data-player");
